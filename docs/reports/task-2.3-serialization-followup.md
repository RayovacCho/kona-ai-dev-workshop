...
这不是无条件的纯改进，而是分配与每对象控制开销之间的权衡。收益更可能出现在高频、
深/宽对象图且 GC 敏感的负载；对没有明显 GC 压力、延迟敏感且每个对象只有少量引用字段的
浅层或扁平对象图，廉价的 TLAB 小数组分配可能比新增分支、边界检查和异常安全控制更便宜。
应用是否获得净收益应同时观察端到端延迟、分配率和 GC，而不能只凭 B/op 推断。
...
此外，独立跨版本程序已验证基线写/最终读、最终写/基线读均成功，且包含共享引用、循环
引用、中英文内容和 unshared 对象的两份输出字节完全一致。
...
## focused A/B/B/A 延迟复测

2026-09-10 进一步执行了此前已准备的预分配输出缓冲和复用对象流基准，顺序为
基线 A1、优化 B1、优化 B2、基线 A2。参数仍为 3 forks、5 次 1 秒预热，但把每次测量从
1 秒延长到 3 秒；两种操作覆盖 `GRAPH`、`GRAPH_CHINESE` 和 `LARGE_OBJECT_ARRAY`。
...
英文 `GRAPH` 在两种操作的两组相邻配对中均同向变慢：预分配操作分别为 +3.17% / +2.03%，
复用流操作分别为 +6.42% / +4.34%；复用流的两轮 99.9% 置信区间也均不重叠。该场景可定量
确认为小幅延迟代价，而不是加速。中文图和大数组仍存在置信区间重叠或顺序漂移，延迟结论
保持不确定。所有目标场景的分配下降均稳定，尤其复用流从按元素线性增长降到约 56 B/op。

这轮结果不改写前述正式 18 项端到端 JMH，只补充回答控制开销问题，并强化“场景相关权衡”
的适用边界。四份原始 JSON、环境与二进制哈希、逐轮点估计和校验和见
[focused A/B/B/A 归档](../../results/task-2.3-focused-abba/README.md)。
...
- [GRAPH 反向顺序复测](../../results/task-2.3-repeat/README.md)：3 条操作路径的原始 JSON
  与 SHA-256，用于核验顺序效应；
- [focused A/B/B/A 复测](../../results/task-2.3-focused-abba/README.md)：预分配缓冲和复用流
  两种操作的四段交错原始 JSON、环境绑定与 SHA-256。

`make check-results` 会验证五组正式结果、一组反向复测和一组 focused A/B/B/A 的校验和、与基准源码版本匹配的
