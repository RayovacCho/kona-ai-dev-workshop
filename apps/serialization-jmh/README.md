...
首次构建会从 Maven Central 下载 JMH 1.37 及其运行依赖。默认配置为 3 个 fork、
5 次预热和 5 次测量，每次 1 秒；结果以 `us/op` 输出，并保存到 `results/` 下的 JSON
文件。`lib/`、`build/` 和原始结果不提交版本库。

本机首次完整结果见[任务 2.1 基准报告](../../docs/reports/task-2.1-serialization-baseline.md)。
